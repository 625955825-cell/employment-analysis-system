  ...
  <div class="page-container">
    <div class="page-header">
      ...
      <div class="header-actions">
        <el-select v-model="selectedYear" placeholder="选择届" clearable style="width: 130px;" @change="loadData" :disabled="!!selectedClassId">
          <el-option v-for="y in years" :key="y" :label="y + '届'" :value="y" />
        </el-select>
        <el-select v-model="selectedClassId" placeholder="选择班级查看" clearable @change="onClassChange" style="width: 200px;">
          <el-option label="全院统计" :value="''" />
      ...
      <el-col :span="6">
        <el-card shadow="hover" class="stat-card">
          <div class="stat-inner blue">
            ...
            <div class="stat-text">
              <p class="stat-label">{{ isDeptView ? '学院总人数' : '班级总人数' }}</p>
...
import { User, CircleCheck, Warning, TrendCharts, Download, Refresh, Reading, Medal, Shop, Promotion } from '@element-plus/icons-vue'
...
const isDeptView = computed(() => selectedClassId.value === null)
...
async function renderMapChart(data) {
  ...
  console.log('[地图] provinceDistribution 原始数据:', provinceData)
  const mapData = Object.entries(provinceData).map(([name, value]) => ({
    name: normalizeProvince(name),
    value
  })).filter(d => d.name !== null)
  console.log('[地图] normalizeProvince 后:', mapData)

  try {
    const response = await fetch('/china.json')
    const chinaJson = await response.json()
    echarts.registerMap('china', chinaJson)
    console.log('[地图] geoJSON 加载成功，省份数:', Object.keys(chinaJson.features || {}).length)

    const maxVal = mapData.length > 0 ? Math.max(...mapData.map(d => d.value)) : 1
    if (mapData.length === 0) {
      console.warn('[地图] 没有可显示的省份数据，尝试显示空地图')
    }
...
async function loadData() {
  ...
  try {
    ...
    if (selectedClassId.value) {
      statsRes = await teacherApi.getClassEmploymentStats(selectedClassId.value, year)
    } else {
      statsRes = await teacherApi.getEmploymentStats(year)
    }
    if (statsRes && typeof statsRes === 'object') {
      stats.value = statsRes
      await nextTick()
      if (mapChart) {
        renderAllCharts(statsRes)
      } else {
        nextTick(() => renderAllCharts(statsRes))
      }
    }
  } catch (e) {
...
async function loadDetail() {
  ...
  try {
    const classId = selectedClassId.value ? String(selectedClassId.value) : null
    const res = await teacherApi.getEmploymentDetail(classId, detailPage.value, detailSize.value)
...
async function exportTable() {
  ...
  const classId = selectedClassId.value ? String(selectedClassId.value) : null
  const res = await teacherApi.getEmploymentDetailAll(classId)
