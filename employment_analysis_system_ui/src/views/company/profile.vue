  ...
  <div class="page-container">
    ...
    <el-card style="margin-top: 20px;" v-loading="loading">
      ...
      <el-descriptions :column="2" border>
        ...
        <el-descriptions-item label="角色">企业用户</el-descriptions-item>
    ...
    <el-card style="margin-top: 16px;" v-loading="loading">
      ...
      <el-descriptions :column="2" border>
        <el-descriptions-item label="企业名称">{{ companyProfile?.companyName || '-' }}</el-descriptions-item>
        <el-descriptions-item label="认证状态">
          <el-tag :type="authType(companyProfile?.authStatus)" size="small">{{ authText(companyProfile?.authStatus) }}</el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="联系人">{{ companyProfile?.contactPerson || '-' }}</el-descriptions-item>
        <el-descriptions-item label="联系电话">{{ companyProfile?.contactPhone || '-' }}</el-descriptions-item>
        <el-descriptions-item label="联系邮箱">{{ companyProfile?.contactEmail || '-' }}</el-descriptions-item>
        <el-descriptions-item label="所属行业">{{ companyProfile?.industry || '-' }}</el-descriptions-item>
        <el-descriptions-item label="企业规模">{{ companyProfile?.scale || '-' }}</el-descriptions-item>
        <el-descriptions-item label="企业性质">{{ companyProfile?.nature || '-' }}</el-descriptions-item>
      </el-descriptions>
    </el-card>
    ...
    <el-dialog v-model="dialogVisible" title="编辑个人信息" width="550px" destroy-on-close>
...
import { profileApi, companyApi } from '@/api'
import { ElMessage } from 'element-plus'
import { Upload, OfficeBuilding } from '@element-plus/icons-vue'
...
async function fetchCompanyProfile() {
  try {
    companyProfile.value = await companyApi.getProfile()
  } catch (e) {
...
async function handleAvatarUpload({ file }) {
  ...
  try {
    ...
    const avatarPath = await profileApi.uploadAvatar(formData)
